...
class VoterListGenerate extends Command
{
    ...
    { 
     ...
     if ($ward_id==0) {
        if ($voterListMaster->is_supplement==0) {
           ...
           $main_page=$this->prepareMainPage($voterListMaster,$WardVillage->id,1);
           $html= $html.$main_page;
        ...
        if ($voterListMaster->is_supplement==0) {
            ...
            $main_page=$this->prepareMainPage($voterListMaster,$ward_id,2);
            $html= $html.$main_page;    
        }else{
    ...
    {  
     ...
     public function prepareMainPage($voterListMaster,$ward_id,$main_page_type)
     {
        ...
        $voterssrnodetails = DB::
          select(DB::raw("Select * From `voters_srno_detail` where `voter_list_master_id` =$voterListMaster->id and `wardid` = $ward_id;"));
